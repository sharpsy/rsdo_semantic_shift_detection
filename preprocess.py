...
def preprocess(input_path, output_path, text_column, nlp):
    ...
    for idx, row in df_data.iterrows():
        ...
        if output is not None:
            ...
            print("Processing text: ", counter, 'Tokenized:', " <eos> ".join(text)[:100], 'Lemmatized:',  " <eos> ".join(lemmatized_text)[:100])
        else:
            print('Discarded row', counter)
    columns = [x for x in df_data.columns if x != text_column] + ['preprocessed_text', 'lemmatized_text']
        ...
        for mw in mw_expressions:
            ...
            if mw_joined in lemmatized_text:
                ...
                if len(text) != len(lemmatized_text):
                    print('error, len text != len lemmas: ', len(text), len(lemmatized_text))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--chunks_column",
                        default='date',
                        type=str,
    ...
    df_data = preprocess(args.data_path, args.data_path.split('.')[0] + '_preprocessed.tsv', args.text_column, nlp)
    ...
    print('Meta information', list(x for x in df_data.columns if x not in [args.text_column, args.chunks_column, 'preprocessed_text', 'lemmatized_text']))
    for idx, row in df_data.iterrows():
        chunk = str(row[args.chunks_column])
        meta = {x:row[x] for x in df_data.columns if x not in [args.text_column, args.chunks_column, 'preprocessed_text', 'lemmatized_text']}
